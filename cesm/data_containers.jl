...
function single_model_result(PARAMETER_SETTINGS, SETS, model)
    ...
    return single_model_result(   
        ...
        solve_time(model),
        sum(value.(model[:p_slack_pos]).data),
        sum(value.(model[:p_slack_neg]).data),
        sum(value.(model[:hp_heat_slack]).data),
        sum(value.(model[:q_chp_region_slack]).data),
...
function initialize_market_results_all(MI)
    ...
    empty_vals = ( Vector{String}(broadcast(string, zeros(MI.PERIODS.N_PERIODS))), # MODELSTATUS
        ...
        Array{Union{Float64,Missing}}(missing,MI.PERIODS.N_PERIODS), # SLACK_EL_POS
        Array{Union{Float64,Missing}}(missing,MI.PERIODS.N_PERIODS), # SLACK_EL_NEG
        Array{Union{Float64,Missing}}(missing,MI.PERIODS.N_PERIODS), # SLACK_HP
        Array{Union{Float64,Missing}}(missing,MI.PERIODS.N_PERIODS), # SLACK_Q
        ...
        Matrix{Union{Float64,Missing}}(missing,1,MI.SETS.TIME[end]) # DUAL ENERGY
    )
...
function append_single_model_result(RESULTS_ALL, MARKET_RESULT, PARAMETER_SETTINGS, idx_period)
    ...
    RESULTS_ALL.AGG_SLACK_EL_POS[idx_period] = MARKET_RESULT.AGG_SLACK_EL_POS
    RESULTS_ALL.AGG_SLACK_EL_NEG[idx_period] = MARKET_RESULT.AGG_SLACK_EL_NEG
    RESULTS_ALL.AGG_SLACK_HP[idx_period] = MARKET_RESULT.AGG_SLACK_HP
    RESULTS_ALL.AGG_SLACK_Q[idx_period] = MARKET_RESULT.AGG_SLACK_Q
    ...
    RESULTS_ALL.DUAL_ENERGY[1,IN_TIME_NO_OVERLAP] = MARKET_RESULT.DUAL_ENERGY[1:PARAMETER_SETTINGS.PERIOD_LENGTH]
